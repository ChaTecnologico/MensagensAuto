...
function carregarAulas() {
  ...
  aulasAtuais.forEach((aula, index) => {
    ...
    aulaDiv.addEventListener('click', function() {
      const checkboxesMarcados = document.querySelectorAll('input[name="aulaSelecionada"]:checked').length;

      // Se o checkbox atual não estiver marcado e já houver 2 marcados, impede a seleção
      if (!input.checked && checkboxesMarcados >= 2) {
        Utils.mostrarNotificacao('Você pode selecionar no máximo 2 atividades.', 'warning');
        return;
      }
...
function gerarMensagem() {
  const selecionadas = document.querySelectorAll('input[name="aulaSelecionada"]:checked'); // Alterado para querySelectorAll
  const mensagemDiv = DOM.obter('mensagem');
  const botoesDiv = DOM.obter('botoes');
  const moduloSelect = DOM.obter('modulo');

  if (selecionadas.length === 0) { // Alterado para verificar o comprimento
    DOM.definirConteudo('mensagem', 'Por favor, selecione ao menos uma aula.');
    DOM.mostrar('botoes', false);
    Utils.mostrarNotificacao('Por favor, selecione ao menos uma aula.', 'warning');
    return;
  }
  ...
  mensagem += 'Acesse o aplicativo *EAD IDEAR* para realizar as seguintes atividades:\n\n';
